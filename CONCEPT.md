...
first payment, and a per-donor daily cap. Paying is an external worker (`21gifts/spend`): this api issues invoices
...
| 2026-08-23 | Spend-worker invoice HTTP: `POST /invoices` fetches a recipient BOLT11 via LNURL-pay; `POST /invoices/proof` accepts the payment preimage. Paying is `21gifts/spend` via lightning.space LNDHub — this api does not store LNDHub credentials or pay. `SPEND_API_TOKEN` optional (503 until set). |
