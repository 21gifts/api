...
| 2026-09-20 | Passkey authenticate/register finish and operator debug session mint refuse a listed duplicate account: HTTP 403 with the wrong-account error and no bearer. `GET /me` with an already-minted token for that id is the same 403 so the client can sign the visitor out. Other authenticated routes treat that token as missing (401). The account row is not deleted.                                                                                                                                                                           |
