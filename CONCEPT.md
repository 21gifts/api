...
| 2026-09-17 | Public Trust Chain picks one incoming kind per subject: `moderator_propose` (if the subject is a `moderator`), else `verify`, else `moderator_appoint`. A founder appoint is omitted when someone already verified the person. **Supersedes** the 2026-09-17 projection that always showed appoint alongside verify. |
...
| 2026-09-17 | Staff Bearer `GET /trust/proposals` lists pending `moderator_propose` (verified subject, no confirm/appoint). Session `GET /trust-chain` still omits a pending propose; once the subject is a `moderator`, that propose is a public chain edge.                                                                                                                                                                                                                                                                                                    |
