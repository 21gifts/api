...
`…/confirm`). When LNDHub env is unset, start still returns 503
(`UnconfiguredInvoicePayer`); when `LNDHUB_*` points at lightning.space,
boot injects `LndhubInvoicePayer`. The process still boots either way.
...
| 2026-08-23 | Recurring-gift payout worker ported in-process: operator env (lightning.space LNDHub + USD recipient JSON + durable JSONL log), daily 20:00 Europe/Zurich, fail-closed Kraken corridor and balance preflight. Donor-upgrade HTTP still later. LNDHub env also injects `LndhubInvoicePayer` for verification. |
