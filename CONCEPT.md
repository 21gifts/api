...
founder seeds; `?around=<id>` returns one hop of stored public edges (no
inferred links).
...
| 2026-09-17 | Public Trust Chain credits the proposer, not the confirmer. Projected kinds are stored `verify` / `moderator_propose` (only once the subject is a `moderator`) / `moderator_appoint`. `moderator_confirm` is omitted. A pending propose (subject still `verified`) stays private and is not a hop neighbor. **Supersedes** the 2026-09-12 public-graph kinds (`moderator_propose` omitted) and the 2026-09-16 neighborhood kinds parenthetical.                                                                                                                         |
