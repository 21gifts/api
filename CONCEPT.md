...
`POST /trust/confirm-moderator`, `POST /trust/appoint-moderator`). Public
`GET /trust-chain` returns stored nodes and edges only (no inferred links).
...
| 2026-09-12 | A 21.gifts-author forum reply opens the member↔member inbox thread with the parent author, copies non-empty reply text into it, and enqueues one targeted Web Push to that author (`/messages?c=` the thread). Top-level notes still broadcast to every other subscriber. Self-replies and Damus-only parents do not notify. Conversation/push failure does not fail `POST /messages`.                                                                                      |
