...
| GET    | `/messages`                                          | Bearer                     | List top-level forum notes (+ visible `replyCount`); 409 if rules missing; name-copy profile notes are omitted and About me text stays       |
...
name stored on the note (case-insensitive) and it has no photo and no
video. A profile note with other About me text stays. Those rows stay
