...
│   │   ├── boot-stores.ts    # DATABASE_URL → auth, optional QueryGiftStore + SqlGiftRecorder, message, contact, conversation, notification, push, BTC-USD and USD-fiat rates, KEK, db_change
...
│   └── gifts-debug.sh        # Operator CLI: list, set role, unlink Lightning Address, messages, video-put, spend (DEBUG_TOKEN)
├── e2e/
