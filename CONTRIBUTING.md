...
│   │   ├── gift-day-claim.ts # One outbound gift per recipient per UTC day
...
│   │   ├── boot-stores.ts    # DATABASE_URL → auth, gifts, day-claim, BTC-USD rates
...
| `DATABASE_URL`         | _(unset → in-memory)_                   | Postgres connection string. When set, auth, `btc_usd_daily`, and `gift_day_claim` are migrated, `GET /gifts` and `GET /gifts/stats` read `gift` plus persisted BTC-USD daily closes (best-effort boot fill; failures log and do not kill the process), and a matching `POST /invoices/proof` inserts into `gift`. Unset keeps `InMemoryAuthStore`, empty gift stats, empty day lists, a no-op gift recorder, and allow-all day claims. |
