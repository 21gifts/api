...
test('GET /debug/api-log with the e2e token lists logs', async ({ request }) => {
  const res = await request.get('/debug/api-log', {
    headers: { authorization: 'Bearer e2e-debug-token' },
  });
  expect(res.status()).toBe(200);
...
test('GET /messages/stats returns a posts total', async ({ request }) => {
  const res = await request.get('/messages/stats');
  expect(res.status()).toBe(200);
  const body = (await res.json()) as {
    postCount: number;
    postsOverTime: { day: string; postCount: number }[];
  };
