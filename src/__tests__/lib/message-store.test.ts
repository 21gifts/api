...
describe('MESSAGE_SCHEMA_SQL', () => {
  it('creates message with photo columns, Nostr columns, index, and additive ALTERs', () => {
    ...
    expect(MESSAGE_SCHEMA_SQL.join('\n')).toMatch(
      /ALTER TABLE message ADD COLUMN IF NOT EXISTS photo_taken_at text/i,
    );
    expect(MESSAGE_SCHEMA_SQL.join('\n')).toMatch(
      /ALTER TABLE message_extra_photo ADD COLUMN IF NOT EXISTS photo_taken_at text/i,
    );
    expect(MESSAGE_SCHEMA_SQL.join('\n')).toMatch(/event_id/);
...
describe('InMemoryMessageStore', () => {
  ...
  it('create round-trips a top-level goalSats and defaults null without one', async () => {
    ...
    expect(withGoal.goalTermDays).toBeNull();
  ...
  it('create round-trips a top-level goalRepayable and stores null without one', async () => {
    ...
    expect(withFlag.goalTermDays).toBeNull();
  ...
  it('create round-trips a top-level goalTermDays and stores null without one', async () => {
    const store = new InMemoryMessageStore();
    const withTerm = await store.create({
      ...EARLY,
      id: 'term',
      goalSats: 21000,
      goalRepayable: true,
  ...
  it('create stores null goalTermDays on a reply even when the row asked for one', async () => {
    const store = new InMemoryMessageStore();
    await store.create(EARLY);
    const child = await store.create({
      ...LATE,
    ...
    expect(child.goalTermDays).toBeNull();
    expect((await store.getById('child-term'))?.goalTermDays).toBeNull();
  });
  ...
  it('create with two stills keeps capture times on photo 0 and extra index 1', async () => {
    const store = new InMemoryMessageStore();
    const photo = { ...JPEG, takenAt: '2025-06-07T08:09:10+02:00' };
    const extra = { ...JPEG2, takenAt: '2025-06-07T08:09:11' };
    const created = await store.create({ ...EARLY, text: '' }, photo, undefined, [extra]);
...
describe('PostgresMessageStore', () => {
    ...
    sql.nextRows = [
      {
        ...
        bytes: 3,
        photo_taken_at: '2025-06-07T08:09:10+02:00',
      },
      {
        message_id: 'm2',
        idx: 2,
    ...
    expect(await store.listExtraPhotoMeta(10)).toEqual([
      {
        ...
        bytes: 3,
        photoTakenAt: '2025-06-07T08:09:10+02:00',
      },
      {
        messageId: 'm2',
        idx: 2,
  ...
  it('create binds a positive goalSats and listLatest maps goal_sats', async () => {
    ...
    expect(listed[0]?.goalTermDays).toBeNull();
  });
  ...
  it('create binds goal_repayable true and listLatest maps it', async () => {
    ...
    expect(sql.executes[0]?.params[32]).toBeNull();
  ...
  it('create binds goal_term_days and listLatest maps it', async () => {
    const sql = new MockSql();
    const store = new PostgresMessageStore(sql);
    const row: MessageRow = {
      id: 'm-term',
    ...
    const created = await store.create(row);
    expect(sql.executes[0]?.params[31]).toBe(true);
    expect(sql.executes[0]?.params[32]).toBe(30);
  ...
  it('create extras INSERT binds message_extra_photo after the message row', async () => {
    ...
    expect(sql.executes[1]?.text).toMatch(
      /\(message_id, idx, photo, photo_content_type, photo_taken_at\)/,
    );
    expect(sql.executes[1]?.text).toMatch(
      /VALUES \(\s*\$1\s*,\s*\$2\s*,\s*\$3\s*,\s*\$4\s*,\s*\$5\s*\)/,
    );
  ...
  it('maps sql capture times from text, arrays, and junk', async () => {
    const sql = new MockSql();
    const store = new PostgresMessageStore(sql);
    const base = {
      id: 'm1',
      ...
      has_photo: true,
      photo_count: 3,
      photo_taken_at: '2026-09-22T11:40:00',
    };
    sql.nextRows = [{ ...base, extra_photo_taken_ats: '["2026-09-22T11:41:00", 1]' }];
    expect((await store.listLatest(1))[0]?.photoTakenAts).toEqual([
      '2026-09-22T11:40:00',
      '2026-09-22T11:41:00',
      null,
    ]);
    sql.nextRows = [{ ...base, extra_photo_taken_ats: 'not-json' }];
    expect((await store.listLatest(1))[0]?.photoTakenAts).toEqual([
      '2026-09-22T11:40:00',
      null,
      null,
    ...
    expect((await store.listLatest(1))[0]?.photoTakenAts).toEqual([]);
    sql.nextRows = [
      { ...base, photo_count: 2, photo_taken_at: null, extra_photo_taken_ats: [null] },
    ];
    expect((await store.listLatest(1))[0]?.photoTakenAts).toEqual([null, null]);
    sql.nextRows = [
      {
        ...base,
        photo_count: 2,
        photo_taken_at: null,
        extra_photo_taken_ats: ['2026-09-22T11:41:00'],
      },
    ];
  ...
  it('getPhoto keeps a stored civil capture time', async () => {
    ...
    expect(photo).toEqual({ ...JPEG, takenAt: '2026-09-22T11:40:00+08:00' });
  });
  ...
  it('getExtraPhoto and listExtraPhotos keep a stored civil capture time', async () => {
    const sql = new MockSql();
    sql.nextRows = [
      {
        photo: JPEG2.bytes,
    ...
    sql.nextRows = [
      {
        ...
        photo: JPEG2.bytes,
        photo_content_type: 'image/jpeg',
        photo_taken_at: '2026-09-22T11:41:00',
      },
    ];
    expect(await store.listExtraPhotos('m1')).toEqual([
      { ...JPEG2, takenAt: '2026-09-22T11:41:00' },
    ]);
  });
...
describe('message payment fiat snapshot', () => {
  ...
  it('adds a later snapshot onto stored fiat and rejects a bad amount', async () => {
    ...
    expect(row?.amountChf).toBe('0.40');
    expect(row?.amountEur).toBe('0.90');
    ...
    expect(crossed?.amountEur).toBe('0.90');
  ...
  it('keeps a stored sibling when a later credit has no dollar amount', async () => {
    ...
    expect(row?.amountEur).toBe('1.00');
    expect(row?.amountPhp).toBe('1.00');
  });

  it('leaves each fiat column alone when extra sats are 0 or that delta is null', async () => {
    const store = new InMemoryMessageStore([
      {
        ...EARLY,
        id: 'm-keep',
    ...
    await store.addSats('m-keep', 0, { usd: '9.00', chf: '9.00', eur: '9.00', php: '9.00' });
    const held = await store.getById('m-keep');
    expect(held?.sats).toBe(21);
    expect(held?.amountUsd).toBe('1.00');
    expect(held?.amountChf).toBeNull();
    expect(held?.amountEur).toBe('0.90');
    expect(held?.amountPhp).toBe('1.00');
    await store.addSats('m-keep', 5, { usd: null, chf: '0.40', eur: null, php: null });
    const row = await store.getById('m-keep');
    expect(row?.sats).toBe(26);
    expect(row?.amountUsd).toBe('1.00');
    expect(row?.amountChf).toBe('0.40');
    expect(row?.amountEur).toBe('0.90');
    expect(row?.amountPhp).toBe('1.00');
  });
...
describe('message fiat accumulator SQL', () => {
  it('keeps a null delta and assigns onto a null column', async () => {
    const sql = new MockSql();
    const store = new PostgresMessageStore(sql);
    await store.addSats('m1', 7, { usd: '1.00', chf: null, eur: '0.90', php: null });
    const added = sql.executes.at(-1)?.text ?? '';
    expect(added).toContain('WHEN $2::bigint = 0 THEN fiat_usd');
    expect(added).toContain('WHEN $3::numeric IS NULL THEN fiat_usd');
    expect(added).toContain('WHEN fiat_usd IS NULL THEN $3::numeric');
    expect(added).toContain('WHEN fiat_chf IS NULL THEN $4::numeric');
    expect(added).not.toContain('THEN NULL');
    expect(added).not.toContain('sats = 0 AND fiat_usd IS NULL');
    await store.recordZapReceipt('evt', 'm1', 7, { usd: null, chf: '1.00', eur: null, php: null });
    const zap = sql.queries.at(-1)?.text ?? '';
    expect(zap).toContain('WHEN inserted.sats = 0 THEN message.fiat_usd');
    expect(zap).toContain('WHEN $4::numeric IS NULL THEN message.fiat_usd');
    expect(zap).toContain('WHEN message.fiat_usd IS NULL THEN $4::numeric');
    expect(zap).toContain('WHEN message.fiat_chf IS NULL THEN $5::numeric');
    expect(zap).not.toContain('THEN NULL');
  });

  it('stores a currency ask and nulls it on a reply', async () => {
    const sql = new MockSql();
    const store = new PostgresMessageStore(sql);
    const row: MessageRow = {
      id: 'm-cur',
    ...
    const mapped = await store.getById('m-cur');
    expect(mapped?.goalCurrency).toBe('USD');
    expect(mapped?.goalAmount).toBe('1.5');
    expect(mapped?.goalAmountUsd).toBe('1.50');
    expect(mapped?.goalAmountEur).toBe('1.40');
    expect(mapped?.goalAmountChf).toBeNull();
    sql.nextRows = [
      {
        id: 'm-cur',
        account_id: 'acc',
    ...
    const unusable = await store.getById('m-cur');
    expect(unusable?.goalAmountUsd).toBeNull();
    expect(unusable?.goalAmountChf).toBeNull();
  });

  it('keeps a currency ask in memory and clears it on a reply', async () => {
    const store = new InMemoryMessageStore();
    const created = await store.create({
      ...EARLY,
      id: 'ask',
    ...
    expect(created.goalCurrency).toBe('USD');
    expect(created.goalAmount).toBe('1.5');
    const child = await store.create({
      ...EARLY,
      id: 'child',
      parentId: 'ask',
