...
- **Purpose:** Stored trust graph. Bearer session required (any role). Bare `GET` returns founder seeds only (`edges` empty) so a large chain is not dumped on first paint. `?around=<id>` returns that chain member plus one hop of stored public edges with at most one incoming kind per subject: `moderator_propose` if subject.role is `moderator`, else `verify`, else `moderator_appoint`; `moderator_confirm` never. Neighborhood uses full sibling lists per subject. A pending propose stays private. Nodes are founder/moderator/verified (never basis). Never invents edges; omits lightning addresses, view keys, and linking keys.
