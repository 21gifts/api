...
- **Purpose:** Bearer staff. Body `{ "accountId" }`. Subject must be `verified`, not self. 409 only when currently pending (latest propose/reject is propose), any confirm/appoint exists, the subject is self, role is not verified, or a concurrent older open propose wins after insert (that extra row is deleted). After a reject, 200 inserts a **new** `moderator_propose` (history kept). When this insert is the oldest open propose and extras exist, delete the latest extra propose and still 200. Role unchanged; logs `trust.moderator_proposed`; `200 { id, name, role }`. After 200, wrap `notifyModeratorProposed` (in-app `moderator_proposal` plus Web Push to other staff). HTTP still 200 if notify fails.
