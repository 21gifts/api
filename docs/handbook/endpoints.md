...
- **Purpose:** Operator catalog of every allowlisted Postgres table as camelCase JSON (cap 200 per table). Media bytes stay off JSON (`photoBytes` / extra-photo `bytes` are lengths). `nostr_nsec_ciphertext` is envelope hex. `btc_usd_daily`, `usd_fiat_daily`, and `db_change` currently dump as empty arrays when no debug list port is wired.
