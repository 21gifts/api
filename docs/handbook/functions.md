...
## Function: nostrNoteUri

- **Purpose:** Build a `nostr:` + NIP-19 `nevent` share URI for a published forum note. Both event id and author pubkey must be exactly 64 lowercase hex (`/^[0-9a-f]{64}$/`, no trim); otherwise null. Relay hints are trimmed, must start with `wss://`, first-wins deduped, and capped at three. An empty filtered list still encodes id and author with no relay hints (kind is never set).
- **Inputs:** `eventId` and `authorPubkey` (`string | null | undefined`) plus a relay URL list (`readonly string[]`).
- **Returns / side effects:** `nostr:nevent1…` or `null`. Encoder is `neventEncode` from `nostr-tools/nip19`; the `nostr:` prefix is added here. No I/O.
- **Used by:** `serializeMessage`.

## Function: publicNoteRelays

- **Purpose:** Relay hints for `nostrNoteUri`. When public publish is on (`NOSTR_PUBLISH_PUBLIC === '1'`), the public write list only (`writeSet.publicUrls`; defaults `wss://relay.damus.io`, `wss://relay.primal.net`, `wss://nos.lol`) — never the durability/space relay, never `writeRelayUrls` (that prepends space), never the search relay (`wss://relay.nostr.band`) and never the indexer (`wss://purplepag.es`). When public publish is off, the one durability relay (`writeSet.spaceUrl`; default `wss://relay.nostr.space`, or `NOSTR_RELAY_SPACE` / `NOSTR_RELAY_URL`). Does not cap; the cap of three lives in `nostrNoteUri`.
- **Inputs:** optional env (`NodeJS.ProcessEnv`; defaults to `process.env`) passed to `resolveWriteSet`.
- **Returns / side effects:** A string array of relay URLs. No I/O.
- **Used by:** `serializeMessage` (via `nostrNoteUri` relay hints).
