...
- **Returns / side effects:** void; logs `nostr.zap.rejected` / `indexed`; records ingest rows; may create a reply and fan out `notifyForumReply` / `notifyZap` in-app to every account except skip (Web Push to bell subscribers); never logs full bolt11. Memory is per store instance and empty after a restart, so the first tick after boot may write one `rejected`/`duplicate` row per known receipt.
