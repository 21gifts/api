...
- **Payment and external-zapper backfills:** Only after `migrateDbChangeSchema` has attached `trg_db_change` to every public table (so the payment backfill's `nostr_zap_payment` inserts are logged), constructs `PostgresMessageStore`, runs `backfillZapPayments`, and immediately runs `backfillExternalZappers` before constructing the remaining Postgres stores and returning. The latter logs `nostr.zapper.backfill.done` with its aggregate counts. In-memory boots call neither backfill.
...
- **External attribution:** Every verified external zap of at least one sat calls `recordZapper` before the block check, so entitlement is retained even when the payer is blocked or the parent is hidden, missing, itself a reply, or otherwise produces no gift-reply. A blocked pubkey stops before `attributeZapReceipt`. Attribution returns `true` for a retry of the same request id on the same receipt, but `false` for a different request id on an already-attributed receipt, for the same request id already attached to another receipt, or for a missing receipt; only a successful attribution can create the deterministic external gift-reply.
- **Gift-reply retries:** `listZapReceiptsAwaitingGiftReply` retries both account (`payerAccountId`) and external (`payerPubkey`) receipts with no `giftReplyId`; missing/hidden/reply parents and missing or blocked payers are dequeued by clearing the applicable payer field. Separately, Postgres boot feeds bounded older indexed-but-unattributed receipts from `listUnattributedIndexedReceipts` to `backfillExternalZappers`; strict verified external requests are attributed and retried there, while account-owned requests remain on the member path.
...
- **Timing:** The caller invokes it only when processing time minus the inbound event time is at most one hour; older replies remain stored without notification.
