...
- **Purpose:** True when a caught driver error is Postgres unique-violation SQLSTATE 23505. Matches node-postgres `code === '23505'` and Bun SQL `errno === '23505'`. `{ code: 'ERR_POSTGRES_SERVER_ERROR' }` with no `errno` is false.
...
- **Returns / side effects:** `{ id, name, text, createdAt, sats, hasPhoto, photoCount, hasVideo, videoContentType, parentId, deletedAt, deletedBy }` with ISO-8601 `createdAt` / `deletedAt` (`deletedAt` JSON `null` when live); `photoCount` is 0–10 (from `row.photoCount` or `hasPhoto ? 1 : 0`). Invalid `createdAt` / `deletedAt` still throws from `toISOString()`. No I/O.
...
- **Returns / side effects:** void; logs `nostr.zap.rejected` / `indexed`; catch logs `nostr.zap.rejected` with `reason: 'error'` plus optional scalar `code`/`errno`/`error` (message truncated to 200 chars; omit empty); records ingest rows; may append a conversation message, create a forum reply, and fan out `notifyZap` in-app to every account except skip, then filtered by each account's `notificationLevel` (Web Push to bell subscribers, same filter); never logs full bolt11. Memory is per store instance and empty after a restart, so the first tick after boot may write one `rejected`/`duplicate` row per receipt that tick still queries (`listLatest`).
