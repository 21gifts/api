...
- **Terminal skip:** Receipts whose remembered decision is terminal (`indexed` or `rejected`/`duplicate`) are skipped before signature check, note lookup, account/LNURL validation, or persist. Other remembered decisions (for example `rejected`/`address`) keep the full path so a receipt can still transition later.
