...
- **Inputs:** `MessageRow` (includes `accountId` and private `authorPubkey`; never photo/video bytes), `payable` boolean, optional `role` (`AccountRole`; omitted for external Nostr authors), optional `replyCount` (top-level `GET /messages` and `GET /members/:accountId/posts` list rows), optional `includeAccountId` (signed-in list/replies/create and member feeds pass true; public GET omits), and last optional `hidden?: { deletedAt: Date; deletedBy: { id, name, role } }`.
