...
- **Sunday rest:** A `Time-Zone` header naming the device IANA zone makes `POST /`, staff `DELETE /:id`, `PATCH /:id/place`, and `POST /:id/invoice` return 403 `{ error: 'SUNDAY_REST' }` while that zone is in Sunday. Pay links, the till, and private messages are not refused. Missing or invalid zone does not refuse.
