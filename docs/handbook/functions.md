...
- **Terminal skip:** Remembered `indexed` and `rejected`/`duplicate` skip signature check, note lookup, account/LNURL validation, and ingest persist. An `indexed` (or `rejected`/`duplicate`) receipt with `gift_reply_id` null still runs `tryEnsureGiftReply` (parent lookup + reply persist). Other remembered decisions (for example `rejected`/`address`) keep the full path so a receipt can still transition later.
