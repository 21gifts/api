...
- **Returns / side effects:** Hono app. 503 if token unset; 401 if bearer mismatches; 200 `{ invoices }` on `GET /invoices`, `{ receiptId, messageId, amountSats }` on `POST /invoices/settle`, and `{ ingests }` on `GET /zap-ingests`. Manual settle accepts `{ paymentHash, note, preimage? }` and delegates to `settleInvoiceManually`; store throws map to 503. Listing is newest-first (cap 200). Logs list/settle results without token, note, preimage, or nsec.
...
- **Checks:** Normalises the payment hash, validates the note and optional preimage, requires an `ok` invoice with positive whole sats and non-empty BOLT11, rejects conversation invoices and missing/hidden messages, then rejects a synthetic receipt or any indexed ingest already matching the payment hash.
- **Returns / side effects:** On success returns `{ ok: true, receiptId, messageId, amountSats }`; otherwise `{ ok: false, reason }`. Credits via `recordZapReceipt`, writes an indexed synthetic 9735 ingest containing `manual=debug-settle`, the note, and only a verified supplied preimage, fans out `notifyZap`, and attempts the payer gift-reply from the original zap request. Logs never contain the note's preimage.
- **Concurrency limit:** The duplicate lookup and `recordZapReceipt` are not one transaction. A real relay receipt indexed at exactly the same instant could count twice; this is an accepted operator-debug limit because manual settlement is performed long after the receipt window.
