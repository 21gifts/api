...
- **Inputs:** `MeRouteDeps` store, `messages`, now, payer, fetchImpl, optional `pushStore`, optional `nostrKek` (required to sign the mint probe), optional `giftStore` and `rates` (defaults empty in-memory; used by `GET /activity`).
...
- **Returns / side effects:** void; logs `nostr.zap.rejected` / `indexed`; records ingest rows; may create a reply and notify the parent author; never logs full bolt11. Memory is per store instance and empty after a restart, so the first tick after boot may write one `rejected`/`duplicate` row per known receipt.
