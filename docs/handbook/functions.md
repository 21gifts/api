...
- **Returns / side effects:** void; logs `nostr.zap.rejected` / `indexed`; catch logs `nostr.zap.rejected` with `reason: 'error'` plus optional allowlisted `name` (error name, ASCII letters only), `code`, and `errno` (ASCII alphanumeric/underscore only); no free-text message field is ever logged; records ingest rows; may append a conversation message, create a forum reply, and fan out `notifyZap` in-app to every account except skip, then filtered by each account's `notificationLevel` (Web Push to bell subscribers, same filter); never logs full bolt11. Memory is per store instance and empty after a restart, so the first tick after boot may write one `rejected`/`duplicate` row per receipt that tick still queries (`listLatest` plus non-null `listReplies` children of those rows).
